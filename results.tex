...
In Figure~\ref{fig:threshold}, we compare the resulting performance by applying \pruning with different precision thresholds on three
models. Tolerating a higher degree of errors gives \pruning more opportunity to remove more neurons to improve the inference time and other
metrics. The improvement reaches a plateau at 15\% of errors for \CNNs, but for \texttt{NMT}, a \RNN model, the reduction of inference time
increases as we give more room for errors. This diagram reinforces our findings that \pruning is more effective on \RNNs than \CNNs. For
the rest discussions of this paper, we use a 5\% \pruning threshold.
...
the compressed model will be. As an example, a 6-bit representation uses \FIXME{xx, and xx} less memory compared to an 8-bit and a 16-bit

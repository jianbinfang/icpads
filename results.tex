...
for Vgg\_16 (a), Resnet\_50 (b) and NMT (c) after using different pruning threshold.
The compression technique to use depends on the optimization target.} 
\label{fig:threshold}
\end{figure*}
...
From Figure~\ref{fig:threshold}a, we see that by removing some of the pathways of the neural network, \pruning can also reduce the model size, although the gain is smaller than \quantization. 

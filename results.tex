...
\hfill
\caption{The change of inference time, accuracy/BLEU, power consumption, energy consumption and memory footprint
for Vgg\_16 (a), Resnet\_50 (b) and \texttt{NMT} (c) when using different pruning thresholds.
The x-axis shows how much loss in the prediction accuracy can be tolerated. }
\label{fig:threshold}
\end{figure*}
...
In Figure~\ref{fig:threshold}, we compare the resulting performance after using different pruning thresholds from 5\% to 75\% on three \CNN
...
quantization does not reduce the inference time but instead it prolongs it. The reasons are described as follows. Data quantization can
speedup the computation (i.e., matrix multiplications) performed on some of the input data by avoiding expensive floating point arithmetics
and enabling SIMD vectorization by using a compact data representation. However, we found that the overhead of the de-quantization process
...
The accuracy is crucially important for any predictive model. A small and faster model is not very useful if it gives wrong predictions all
the time.
...
For \pruning, the Figure~\ref{fig:analy_prun}c compares the reduction in the top-1 and the top-5 scores for \texttt{Vgg\_16} and
...
Figures~\ref{fig:analy_quan}f and \ref{fig:analy_prun}f show other three evaluation metrics of the considered deep learning models after
...
\hfill
\subfloat[][Energy consumption]{\includegraphics[width=0.23\textwidth]{figure/q_p_energy.pdf}}
\hfill

...
\cparagraph{Deep Learning Models.} We consider 10 pre-trained \CNN models for image recognition from the TensorFlow-Slim library~\cite{silberman2013tensorflow} and a recurrent neural network (\RNN) model for machine translation. Table~\ref{tab:workload} lists
...
\FIXME{
In this paper, we only apply the quantization technique on 10 \CNN models,
as Tensorflow does not support to quantize the \RNN models yet. 
Further more, the compressed models need to retrain to gain the accuracy, which takes
much time, especially for pruning technique, 
and this paper present 3 typical models for pruning technique.
}
